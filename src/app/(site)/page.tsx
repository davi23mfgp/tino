...
export default async function Vitrine() {
  ...
  return <div className="lp">
    ...
    <main id="conteudo">
      ...
      <section className="lp-secao lp-alternada" id="visao"><Revelar className="lp-secao-texto"><h2>Encontre cada gasto.<br />Entenda o todo.</h2><p>Busque lançamentos, filtre por conta ou categoria e veja os totais do período. O detalhe aparece quando você precisa.</p><Comecar texto="Organizar meu dinheiro" /></Revelar><Revelar className="lp-moldura-recurso"><img src="/demonstracao/extrato-desktop.png" alt="Extrato real do Tino com busca, filtros e totais" width={1265} height={712} loading="lazy" /></Revelar></section>
      ...
      <section className="lp-secao lp-precos" id="planos"><Revelar className="lp-central"><h2>Um plano para você.<br />E para onde quer chegar.</h2><p>Experimente por {dias} dias, sem cartão para começar. Escolha o plano que faz sentido para sua rotina.</p></Revelar><Precos planos={planos} dias={dias} /></section>
    ...
    <footer className="lp-secao lp-rodape"><Link href="/" className="lp-marca"><Leao tamanho={38} /><span>tino.</span></Link><p>Seu dinheiro, mais simples.</p><nav aria-label="Rodapé"><a href="#recursos">Recursos</a><a href="#planos">Preços</a><Link href="/termos">Termos</Link><Link href="/privacidade">Privacidade</Link><Link href="/login">Entrar</Link></nav><small>© {new Date().getFullYear()} Tino</small></footer>
  </div>
}
