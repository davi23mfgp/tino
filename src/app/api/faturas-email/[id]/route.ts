...
export const DELETE=comSessao<Contexto>(async(s,r,c)=>{const {id}=await c.params;const atualizados=await prisma.faturaRecebida.updateMany({where:{id,larId:s.larId},data:{status:"CONCLUIDA",conteudo:Buffer.alloc(0)}});if(!atualizados.count)throw new ErroDeUso("Arquivo não encontrado.",404);return ok({concluida:true})})
