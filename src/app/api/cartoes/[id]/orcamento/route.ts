...
export const GET = comSessao<Contexto>(async (sessao, req, contexto) => {
  ...
  if (!competencia || !/^\d{4}-\d{2}$/.test(competencia)) throw new ErroDeUso("Informe o mês.")
  ...
  return ok(plano ?? { totalCentavos: conta.orcamentoMensalCentavos, categorias: [] })
})
...
export const PUT = comSessao<Contexto>(async (sessao, req, contexto) => {
  ...
  if (!/^\d{4}-\d{2}$/.test(dados?.competencia ?? "") || !dinheiroValido(dados?.totalCentavos)) throw new ErroDeUso("Informe um orçamento válido.")
  const categorias = dados.categorias ?? []
  if (categorias.some((linha) => !linha.categoriaId || !dinheiroValido(linha.limiteCentavos))) throw new ErroDeUso("Revise os valores por categoria.")
