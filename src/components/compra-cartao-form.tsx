...
export function CompraCartaoForm({contaId,categorias,compra,parcelamento,fechar,salvou,embutido=false}:{contaId:string;categorias:{id:string;nome:string}[];compra?:CompraCartao;parcelamento?:CompraParcelada;fechar:()=>void;salvou:()=>void;embutido?:boolean}) {
  ...
  const formulario=<form onSubmit={salvar} className="grid gap-4 px-4 py-4 sm:px-6 sm:py-5 sm:grid-cols-2">
    ...
    {!compra&&!parcelamento&&Number(quantidade)===1&&<label className="block text-sm">Fatura (opcional)<Input type="month" value={fatura} onChange={e=>setFatura(e.target.value)}/><small className="text-muted-fg">Em branco: calcular pelo fechamento.</small></label>}
  ...
  return <Dialog open onOpenChange={a=>{if(!a&&!ocupado)fechar()}}><DialogContent><DialogHeader><DialogTitle>{compra||parcelamento?"Editar compra":"Nova compra"}</DialogTitle><DialogDescription>{parcelamento?"Atualize a descrição e as parcelas pagas.":"Registre no cartão selecionado."}</DialogDescription></DialogHeader>{formulario}</DialogContent></Dialog>
}
