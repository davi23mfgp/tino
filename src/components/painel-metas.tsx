...
export async function PainelMetas({reserva=false}:{reserva?:boolean}) {
 ...
 // A mesma que a API de reserva ajusta: a mais antiga entre as ativas ou pausadas.
 const metaReserva=reserva?[...metas].filter(m=>m.status==="ATIVA"||m.status==="PAUSADA").sort((a,b)=>a.criadoEm.getTime()-b.criadoEm.getTime())[0]:undefined
