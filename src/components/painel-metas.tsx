...
export async function PainelMetas({reserva=false}:{reserva?:boolean}) {
 ...
 return <div className="space-y-5">{!reserva&&<Abertura
  rotulo="Metas"
  titulo={maisPerto
   ? <>Faltam <em>{formatarMoeda(maisPerto.alvoCentavos-maisPerto.saldoCentavos)}</em> para {maisPerto.nome}.</>
   : ativas.length===0&&metas.length>0
     ? <>Todas as suas metas estão completas.</>
     : <>Qual é seu próximo objetivo?</>}
  apoio={maisPerto
   ? <>{ativas.length} {ativas.length===1?"meta ativa":"metas ativas"}{maisPerto.aporteMensalCentavos>0?<> · {formatarMoeda(maisPerto.aporteMensalCentavos)} por mês nesta</>:null}</>
