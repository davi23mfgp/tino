  ...
  return <header className="app-header">
    ...
    <div className="app-header-actions">
      ...
      {!apenasLoja && <Sheet open={aberto} onOpenChange={setAberto}>
        <SheetTrigger asChild><button aria-label={novas.length?`Notificações, ${novas.length} ${novas.length===1?"não lida":"não lidas"}`:"Notificações"} className="relative grid size-11 place-items-center rounded-full border border-pauta"><Bell className="size-5" aria-hidden/>{novas.length>0 && <span className="absolute right-1 top-1 size-2 rounded-full bg-acao" />}</button></SheetTrigger>
